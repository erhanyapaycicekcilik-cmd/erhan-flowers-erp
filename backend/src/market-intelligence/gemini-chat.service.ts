...
export class GeminiChatService {
  ...
  async chat(message: string, history: ChatMessage[] = []): Promise<{ reply: string }> {
    ...
    try {
      ...
      this.logger.error(`Gemini sohbet isteği başarısız. model=${model}`, error instanceof Error ? error.stack : error);
      throw new ServiceUnavailableException('Gemini sohbet isteği tamamlanamadı. Backend loglarında detaylı hata kaydı oluşturuldu.');
    }
  }
